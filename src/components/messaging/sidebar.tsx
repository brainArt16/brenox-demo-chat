...
export function Sidebar({
  ...
  return (
    <div className="flex h-full w-full flex-col overflow-hidden border-r border-gray-200 bg-white md:w-80 lg:w-96">
      ...
      {showOnlineSection && (
        <div className="border-b border-gray-100 px-4 py-4 sm:px-6">
          <div className="mb-4 flex items-center justify-between">
            <h3 className="text-sm font-medium text-gray-500">{onlineLabel}</h3>
          </div>
          {onlineUsers.length === 0 ? (
            <p className="text-xs text-gray-400">No one else online yet</p>
          ) : (
        ...
        {conversations.length === 0 ? (
          ...
          conversations.map((conv) => (
            <button
              ...
              className={`w-full cursor-pointer border-b border-gray-50 px-4 py-3 text-left transition hover:bg-gray-50 ${
